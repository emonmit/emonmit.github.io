...
## 数据量再大的话
现在只是一万条十万条的批量，如果提升到千万条几亿条的话，还能用一样的方法吗？大方向肯定是不变的，之前说的那些点一样适用。另外要注意的是：由于时间开销变大，不可能继续把这部分放在前端展示出来，给个菊花图让用户等；而应该用脚本在后端完成插入操作。若插入的数据表数据量很大的话应该要考虑分表。这方面我也没有相应的经验，如果有好的建议指导或者说有写的不对的地方，欢迎留言或联系我 QQ:418584492。
